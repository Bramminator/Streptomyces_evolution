...
double initial_rearrangement_rate=0.015;
double mut_step_rearrangement = 0.1;
double mut_rate_rearrangement = 0.3;
...
void Initial(void)
{
  ...
  for(int i = 0; i < (int)argc_g; i++) // Loops over the words on the command line
  {
    ...
    if(strcmp(readOut, "-ir") == 0) initial_rearrangement_rate = atof(argv_g[i+1]);
    if(strcmp(readOut, "mrr") == 0) mut_rate_production = atof(argv_g[i+1]);
    if(strcmp(readOut, "msr") == 0) mut_step_rearrangement = atof(argv_g[i+1]);
  }
...
void InitialPlane(void)
{
    ...
    printf("%f", chance);
		InitialSetS(Medium, 2, bg, s1, chance, s3, chance);
 ...
 void NextState(int row, int col)
 {
   ...
   if(Medium[row][col].val == 0 && neighbor.val == 1){
     if(genrand_real1() < growthrate){
       if(genrand_real1() > neighbor.fval3){ // WT reproduction
         ...
         if(genrand_real1() < mut_rate_rearrangement){
           Medium[row][col].fval3 += mut_step_rearrangement * (genrand_real2()/100 - 0.005);
           if(Medium[row][col].fval3 < 0){
             Medium[row][col].fval3 = 0;
           }
         }
      }
...
void Update(void)
{
  ...
  if(Time%1000000 == 0){
    ...
    sprintf(restartbuffer,"%s/grid_restart%d.txt",gridfolder,Time);
    restartfile = fopen(restartbuffer, "a");
    for(i=1;i<=nrow;i++)
    {
      for(j=1;j<=ncol;j++)
      {
        fprintf(restartfile, "%d\t%f\t%f\t%f\t%f\t%d\t%d\n", Medium[i][j].val, Medium[i][j].fval, Medium[i][j].fval2, Diffusion_plane[i][j].fval, Medium[i][j].fval3,i, j);
      }
    }
    fclose(restartfile);
  }
  ...
  if(Time%1000 == 0){
    ...
    if(Time == 0){
      fprintf(costsfile, "Time\tCost\tConcentration\tProduction\tMut_rate\tI\tJ\n");
      }
    for(int r=1;r<=120;){
       i = genrand_int(1,nrow);
       j = genrand_int(1,ncol);
       if(Medium[i][j].val > 0 && Medium[i][j].val != 3){
         fprintf(costsfile, "%d\t%f\t%f\t%f\t%f\t%d\t%d\n", Time, Medium[i][j].fval,Diffusion_plane[i][j].fval, Medium[i][j].fval2, Medium[i][j].fval3,i,j);
         r +=1;
       }
     }
    fclose(costsfile);
  ...
  if(Time%killing_time == 0){
    ...
    if(Time%1000000-2500 ==0 || Time%1000000-5000==0 || Time%1000000-7500==0)
    {
      ...
      rearrangement_file = fopen(mutationbuffer, "a");
      for(i=1;i<=nrow;i++)
      {
        for(j=1;j<=ncol;j++)
        {
          fprintf(rearrangement_file, "%f\t%d\t%d\n", Medium[i][j].fval3, i, j);
        }
      }
      fprintf(rearrangement_file, "\n");
      fclose(rearrangement_file);
    }
